...
$stmt = $pdo->prepare("SELECT  p.*, c.id_user_ask, c.id_post FROM postes p JOIN collabs c ON p.id = c.id_post WHERE (p.id_user_request = :current_user_id AND p.id_user_travail IS NOT NULL) OR (p.id_user_travail = :current_user_id AND p.id_user_request IS NOT NULL)");
